...
```
frontend/index.html (landing — escolha: motorista / minha conta / administrador)
        │
        ├── Totem (frontend/totem/)           self-service na estação, sem login
        ├── App do motorista (frontend/app/)  login por placa, histórico, chat
        └── Dashboard (frontend/dashboard/)   login admin, dado agregado
                        │  HTTP/JSON (todos os três)
                        ▼
              API Flask (backend/api_server.py)
                        │
                        ▼
         Motor (backend/ev_chargegrid.py)
                        │
                        ▼
        Postgres — Supabase (DATABASE_URL no .env, não versionado)
                        ...
                        │
        Chatbot — chatbot/chatbot.py (local) OU
...
backend/                              motor, API e o modelo de IA — tudo que roda em Python no servidor
...
├── tests/
│   └── test_ev_chargegrid.py         testes do motor (tarifação, DLB, hash/mascaramento, regressão da demo)
...
frontend/                             as 3 telas + landing page — HTML/CSS/JS puro, sem build
├── index.html                        landing page — escolhe motorista / minha conta / admin
├── totem/                            totem do motorista (self-service, fixo na estação)
│   └── index.html / app.js / style.css
├── app/                              área pessoal do motorista — login por placa
│   └── index.html / app.js / style.css
└── dashboard/                        painel do gestor — login admin
    └── index.html / app.js / style.css

chatbot/                              assistente conversacional (RAG + Groq)
├── chatbot.py                        chatbot em terminal, local, sem Colab (mesma lógica do notebook)
├── guardrails.py                     filtro de prompt injection / uso indevido, roda antes do modelo
...
└── tests/
    ├── test_chatbot.py               testes da lógica pura do chatbot (offline)
    └── test_guardrails.py            testes dos guardrails (offline)
```
