...
- **`render.yaml`** (raiz do repo): blueprint do [Render](https://render.com) — aponta pra `backend/`, instala as dependências e sobe com `gunicorn` (servidor de produção; o servidor embutido do Flask, usado localmente, avisa explicitamente que não deve ser exposto assim).
- **`backend/Procfile`**: mesma ideia, pra Railway ou qualquer host compatível com Procfile.
