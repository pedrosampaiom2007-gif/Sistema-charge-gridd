...
project/
├── backend/
│   ├── ev_chargegrid.py   ← código original do Raul (+1 função nova, ver abaixo)
...
│   ├── style.css
│   └── app.js
└── totem/                 ← tela self-service do MOTORISTA, fixa na estação
    ├── index.html
    ├── style.css
    └── app.js
```

## Totem (self-service, na estação)

Fluxo: **Livre → digita placa → carregando (ao vivo) → encerrar → QR Pix → recibo → volta pra livre.**

- Cada totem físico serve **uma única estação**. Defina qual via URL:
  `totem/index.html?estacao=3` (padrão é a estação 1 se omitido).
- Reaproveita os mesmos endpoints do dashboard (`/api/sessoes/iniciar`,
  `/api/sessoes/<n>/encerrar`, `/api/painel`) — nenhuma rota nova precisou
  ser criada no backend para isso além do timestamp de início (ver abaixo).
- O QR code é gerado **no navegador**, a partir do `checkout_url` que
  `criar_pagamento_sandbox()` do Raul já devolve — usa a lib `qrcodejs`
  (CDN, só client-side, não expõe a URL de pagamento pra terceiros).
- Como o gateway é simulado, o backend confirma o pagamento
  instantaneamente; o totem segura a tela de "aguardando Pix" por ~3s só
  pra parecer uma espera real. Isso é só estética — não é um comportamento
  do backend, é um `setTimeout` no `totem/app.js`.
- Pequeno complemento em `api_server.py` (não em `ev_chargegrid.py`): um
  dicionário `_INICIO_REAL` guarda o timestamp exato de início de cada
  sessão, porque o backend original só guarda a hora (0–23), não o
  instante — precisa disso pro cronômetro "tempo decorrido" do totem.
