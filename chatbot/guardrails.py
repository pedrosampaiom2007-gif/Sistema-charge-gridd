"""
guardrails.py — defesa do chatbot contra prompt injection e contra pergunta que
ele nao deve responder. Trazido da Sprint 3 pro sistema principal.

Sao tres coisas, nesta ordem:

  detectar_injection(pergunta)      -> rotulo do ataque, ou None se passou
  avaliar_escopo(pergunta)          -> "ok" | "fora_de_escopo" | "dominio_restrito"
                                       | "comparacao_produto"
  resposta_parece_vazamento(texto)  -> True se a resposta do modelo vazou o prompt
...
# ==========================================================================
# Escopo: assunto do chatbot e dominios que exigem profissional habilitado
# ==========================================================================
...
def _norm_escopo(texto: str) -> str:
    sem_acento = unicodedata.normalize("NFKD", texto.lower())
    return "".join(c for c in sem_acento if not unicodedata.combining(c))


def _tokens_escopo(texto: str) -> set[str]:
    """Palavras inteiras, sem acento. Evita 'acao' casar dentro de 'estacoes'."""
    palavra: list[str] = []
    out: set[str] = set()
    for c in _norm_escopo(texto):
        if c.isalnum():
            palavra.append(c)
        elif palavra:
...
def avaliar_escopo(pergunta: str) -> ResultadoEscopo:
    alvo = _norm_escopo(pergunta)
    toks = _tokens_escopo(pergunta)
